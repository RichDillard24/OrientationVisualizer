...
struct OrientationCubeView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> SCNView {
        ...
        let cube = SCNBox(width: 1, height: 1, length: 1, chamferRadius: 0.0)
        ...
        let light = SCNNode(); light.light = SCNLight(); light.light?.type = .omni; light.position =
            SCNVector3(2,2,4)
    ...
    func updateUIView(_ v: SCNView, context: Context) {
        context.coordinator.node?.orientation = SCNVector4(Float(qw), Float(qy), Float(qz), Float(qw))
    }
